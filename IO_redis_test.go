...
//)
//
//func TestIO_OneVirtualMediaFile_Redis(t *testing.T) {
...
//}
//
//func TestIO_MultipleVirtualMediaFileConsecutively_Redis(t *testing.T) {
//	homePath, err := os.UserHomeDir()
//	assert.Equal(t, nil, err)
//	_ = utils.DeleteFile(homePath + "/" + fsPath)
//	_ = utils.DeleteFile(homePath + "/" + headerPath)
...
//	}
//
//	err = provider.CloseArchiver(homePath)
//	assert.Equal(t, nil, err)
//
//	arch2, err := provider.ParseFileSystem(fsID,homePath, &eventListener, log.GetScope("test"), redisOptions)
//	assert.Equal(t, nil, err)
//
//	for i, pckts := range packets {
//		vf2, err := arch2.OpenVirtualMediaFile(vmIDs[i])
//		assert.Equal(t, nil, err)
//
//		for _, v := range pckts {
